...
def show_command_logs(message):
    ...
    if user_id in users:
        ...
        response = "❌ YE BOT FREE NAHI H JISE LENA H BUY KARO @rajaraj_04❌!!"
...
def generate_custom_key_command(message):
    user_id = str(message.chat.id)
    if user_id in admin_id:
        command = message.text.split()
        if len(command) >= 3:
            try:
                custom_code = command[1]
                duration_value = int(command[2][:-1])  # Extract the number (e.g., '10d' -> 10)
                duration_unit = command[2][-1].lower()  # Extract the unit (e.g., '10d' -> 'd')
                max_uses = int(command[3]) if len(command) == 4 else 1

                # Convert the duration to days or minutes
                if duration_unit == 'd':
                    expiration_date = add_time_to_current_date(days=duration_value)
                elif duration_unit == 'm':
                    expiration_date = (datetime.datetime.now() + datetime.timedelta(minutes=duration_value)).strftime('%Y-%m-%d %H:%M:%S')
                else:
                    raise ValueError("Invalid time unit, use 'd' for days or 'm' for minutes.")

                # Store the key
                keys[custom_code] = {
                    "expires": expiration_date,
                    "max_uses": max_uses,
                    "uses": 0  # Track the usage count
                }
                save_keys()

                response = f"✅ 𝐊𝐞𝐲 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐝:\n🔑 {custom_code}\n📅 Expires on: {expiration_date}\n🔢 Max Uses: {max_uses}"
            except ValueError:
                response = "𝐄𝐫𝐫𝐨𝐫: Please provide a valid duration (e.g., 10d for 10 days or 15m for 15 minutes)."
        else:
            response = "𝐔𝐬𝐚𝐠𝐞: /gen [custom_code] <days/minutes> [max_uses]"
    else:
        response = "𝐎𝐧𝐥𝐲 𝐀𝐝𝐦𝐢𝐧𝐬 𝐂𝐚𝐧 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞 𝐊𝐞𝐲𝐬."
